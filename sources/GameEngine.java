...
//            }
    ...
    // Key press update
    public void keyPressUpdate(KeyEvent e){
        if (e.getCode() == KeyCode.UP) {
            if (secondPlayer.getView().getTranslateY() >= 5)
                secondPlayer.moveForwards();
            else
    ...
    // Key release update
    public void keyReleaseUpdate(KeyEvent e){
        if (e.getCode() == KeyCode.UP) {
        ...
        if (gameObject.isFireReady()) {
            switch(gameObject.getFirePowerLevel()) {
                case 0: Bullet bullet = new Bullet(new ImageView(sprites.getValue("bullet")));
                    ...
                    playMedia(sounds.getValue("fire"));
                    break;
                case 1:
            }
        }
            ...
            for (GameObject asteroid : returnObjects1) {
                if (player.intersects(asteroid)) {
                    ...
                    if (player.getHealth() <= 0)
                        resetPlayer(player);
                    playMedia(sounds.getValue("explosion"));
                    deadAsteroids.add(asteroid);
                    asteroid.setDead();
            ...
            for (GameObject enemy : returnObjects1) {
                if (bullet.intersects(enemy)) {
                    ...
                    if(enemy.getHealth() <= 0){
                        ...
                        animations.add(new Animation("sources/Sprites/explosion2 sheet",enemy,4));
                    }
                }
            }
            returnObjects1.clear();
        ...
        playerBullets.forEach(bullet -> {
            ...
            for (GameObject asteroid : returnObjects1) {
                if (bullet.intersects(asteroid)) {
                    ...
                    animations.add(new Animation("sources/Sprites/explosion",asteroid,3));
                }
            }
            returnObjects1.clear();
            asteroids.removeAll(deadAsteroids);
